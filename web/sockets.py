...
def init_socketio(socketio):
    ...
    def handle_read_file(data):
        ...
        if not path or not os.path.isfile(path):
            emit('file_text', {"client_id": client_id, "text": "文件不存在", "is_base64": False})
            return
        ...
        try:
            ...
            try:
                ...
                emit('file_text', {"client_id": client_id, "path": path, "text": text, "is_base64": False})
            except UnicodeDecodeError:
                # 如果不是文本文件，base64 返回
                b64 = base64.b64encode(content).decode()
                emit('file_text', {"client_id": client_id, "path": path, "text": b64, "is_base64": True})

        except Exception as e:
            emit('file_text', {"client_id": client_id, "text": f"读取失败: {e}", "is_base64": False})
