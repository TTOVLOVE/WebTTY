...
from ..services.encryption import create_secure_socket
...
def reliable_send(sock, data_dict):
    ...
    try:
        if hasattr(sock, 'send_encrypted'):
            # 使用加密Socket发送
            return sock.send_encrypted(data_dict)
        else:
            # 回退到明文发送
...
def receive_thread(conn, client_id, app, addr, stop_event):
    """专门用于接收客户端数据并处理的线程（支持加密）。"""
    while not stop_event.is_set():
        try:
            # 使用安全Socket接收消息
            if hasattr(conn, 'receive_encrypted'):
                messages = conn.receive_encrypted()
                if messages is None:
                ...
                for data in messages:
                    process_client_message(data, client_id, app, addr, conn, stop_event)
            ...
            print(f"[错误] 处理客户端 {client_id} 消息时发生错误: {e}")
            continue
    print(f"Receive thread for client {client_id} finished.")


def process_client_message(data, client_id, app, addr, conn, stop_event):
    """处理客户端消息"""
    try:
        # Process data...
...
def client_handler(conn, addr, client_id, app):
    ...
    # 创建安全Socket包装器
    secure_conn = create_secure_socket(conn)
    
    # 执行密钥交换（服务端模式）
    print(f"[调试] 开始与客户端 {client_id} 进行密钥交换...")
    if not secure_conn.perform_key_exchange(is_server=True):
        print(f"[错误] 与客户端 {client_id} 的密钥交换失败")
        conn.close()
        return
    
    print(f"[+] 与客户端 {client_id} 的加密通道已建立")
    
    q = client_manager.register_client(client_id, secure_conn, addr)
    ...
    # 创建并启动发送和接收线程（使用安全连接）
    sender = threading.Thread(target=send_thread, args=(secure_conn, q, client_id, stop_event))
    receiver = threading.Thread(target=receive_thread, args=(secure_conn, client_id, app, addr, stop_event))
    ...
    client_manager.remove_client_if_match(client_id, secure_conn)
