...
class ClientEncryptionManager:
    ...
    def generate_keypair(self):
        ...
        try:
            ...
            logger.info("[客户端] ECDH密钥对生成成功")
            return True
        except Exception as e:
            logger.error(f"[客户端] 生成ECDH密钥对失败: {e}")
            return False
    ...
    def get_public_key_bytes(self):
        ...
        try:
            ...
            logger.error(f"[客户端] 序列化公钥失败: {e}")
            return None
    
    def load_peer_public_key(self, peer_public_key_b64):
        """加载服务端的公钥并计算共享密钥"""
        try:
            logger.info(f"[客户端] 开始加载服务端公钥: {peer_public_key_b64[:50]}...")
            ...
            logger.info("[客户端] 服务端公钥解码成功")
            
            # 计算共享密钥
            shared_key = self.private_key.exchange(ec.ECDH(), peer_public_key)
            logger.info(f"[客户端] 共享密钥计算成功，长度: {len(shared_key)}")
            ...
            logger.info("[客户端] 密钥交换完成，加密通道已建立")
            logger.info(f"[客户端] AES密钥长度: {len(self.aes_key)}, is_initialized: {self.is_initialized}")
            return True
            
        except Exception as e:
            logger.error(f"[客户端] 密钥交换失败: {e}")
            return False
    
    def encrypt_message(self, message_dict):
        """加密消息"""
        if not self.is_initialized:
            raise RuntimeError("[客户端] 加密管理器未初始化")
    ...
    def process_handshake_response(self, response):
        ...
        try:
            if response.get('type') != 'key_exchange_ack':
                logger.error(f"[客户端] 握手响应类型错误: {response.get('type')}")
                return False
            
            peer_public_key = response.get('public_key')
            if not peer_public_key:
                logger.error("[客户端] 握手响应中缺少公钥")
                return False
            
            return self.load_peer_public_key(peer_public_key)
            
        except Exception as e:
            logger.error(f"[客户端] 处理握手响应失败: {e}")
            return False


class ClientSecureSocket:
    """客户端安全Socket包装器，提供透明的加密/解密功能"""
    ...
    def send_encrypted(self, message_dict):
        """发送加密消息"""
        try:
            logger.info(f"[客户端] 准备发送消息: {message_dict}")
            logger.info(f"[客户端] 加密管理器状态 - is_initialized: {self.encryption_manager.is_initialized}")
            ...
            if self.encryption_manager.is_initialized and not is_key_exchange:
                ...
                logger.info(f"[客户端] 发送加密消息，payload长度: {len(payload)}")
            else:
                # 如果加密未初始化或是密钥交换消息，发送明文
                payload = json.dumps(message_dict, ensure_ascii=False).encode('utf-8') + b'\n'
                logger.info(f"[客户端] 发送明文消息，payload长度: {len(payload)}")
            
            self.socket.sendall(payload)
            logger.info("[客户端] 消息发送成功")
            return True
            
        except Exception as e:
            logger.error(f"[客户端] 发送加密消息失败: {e}")
            return False
    ...
    def receive_encrypted(self, buffer_size=4096):
        ...
        try:
            ...
            if not data:
                logger.info("[客户端] 接收到空数据，连接可能已关闭")
                return None
            
            logger.info(f"[客户端] 接收到数据，长度: {len(data)}")
            self.buffer += data
            messages = []
            ...
            while b'\n' in self.buffer:
                ...
                try:
                    ...
                    logger.info(f"[客户端] 解析消息成功: {message_dict}")
                    
                    # 如果是加密消息且加密管理器已初始化，则解密
                    if (message_dict.get('encrypted') and 
                        self.encryption_manager.is_initialized):
                        logger.info("[客户端] 开始解密消息")
                        decrypted_message = self.encryption_manager.decrypt_message(message_dict)
                        logger.info(f"[客户端] 解密成功: {decrypted_message}")
                        messages.append(decrypted_message)
                    else:
                        # 明文消息（握手阶段或未加密）
                        logger.info("[客户端] 处理明文消息")
                        messages.append(message_dict)
                        
                except (json.JSONDecodeError, UnicodeDecodeError) as e:
                    logger.error(f"[客户端] 解析消息失败: {e}")
                    continue
                except Exception as e:
                    logger.error(f"[客户端] 解密消息失败: {e}")
                    continue
            
            # 返回消息列表
            logger.info(f"[客户端] 本次接收处理了 {len(messages)} 条消息")
            return messages
            
        except Exception as e:
            logger.error(f"[客户端] 接收加密消息失败: {e}")
            return None
    
    def perform_key_exchange(self):
        """执行客户端密钥交换"""
        try:
            logger.info("[客户端] 开始密钥交换")
            
            # 客户端：发起密钥交换
            handshake_msg = self.encryption_manager.create_handshake_message()
            if not handshake_msg:
                logger.error("[客户端] 创建握手消息失败")
                return False
            
            logger.info(f"[客户端] 发送握手消息: {handshake_msg}")
            
            # 发送握手消息
            if not self.send_encrypted(handshake_msg):
                logger.error("[客户端] 发送握手消息失败")
                return False
            
            # 等待服务端响应
            logger.info("[客户端] 等待服务端响应...")
            messages = self.receive_encrypted()
            if not messages:
                logger.error("[客户端] 未收到服务端响应")
                return False
            
            response = messages[0]
            logger.info(f"[客户端] 收到服务端响应: {response}")
            
            result = self.encryption_manager.process_handshake_response(response)
            if result:
                logger.info("[客户端] 密钥交换成功完成")
            else:
                logger.error("[客户端] 密钥交换失败")
...
def create_secure_socket(socket_obj):
    """创建客户端安全Socket包装器"""
    return ClientSecureSocket(socket_obj)